...
//[[Rcpp::depends(RcppArmadillo)]]
//[[Rcpp::export]]
Rcpp::List BEnpl(Rcpp::NumericMatrix X,Rcpp::NumericVector Y,double a1_shape,double b1_rate,double a2_shape,double b2_rate, double sigsc_hyper, double sigsh_hyper,int niter, int nburn,int nthin)
{
...
// 0. Variable Declaration
   //     RNGScope scope;
        ...
        Rcpp::NumericVector Lambda1square(ncol) ;
        ...
        Rcpp::NumericVector Lambda1hist(N);
...
// 2. Starting Values
        ...
        double lambda1_sq   = R::rgamma(a1_shape,1/b1_rate);
        double lambda2   = R::rgamma(a2_shape,1/b2_rate);
//        double lambda_sq = std::pow(lambda,2);

        for ( int i =0 ; i < ncol;i++)
        {
            tau_inv[i] = 1/R::rgamma(1,2/lambda1_sq) ;
            Beta[i]= 0 ;
            Lambda1sq[i] = lambda1_sq;


        }

        tau_inverse =    Rcpp::as<arma::colvec>(tau_inv) ;
        dtau_inv   =    arma::diagmat(tau_inverse) +arma::diagmat(lambda2*onecol);
...
//int i=1;
    ...
    Lambda1sq     =   onecol*lambda1_sq;
//*/
    mu         =    arma::sqrt(sigma2*Lambda1sq%arma::pow(Beta2,-1)) ;
    Mu         =    Rcpp::as<Rcpp::NumericVector>(Rcpp::wrap(mu)) ;


    Lambda1square =  Rcpp::as<Rcpp::NumericVector>(Rcpp::wrap(Lambda1sq));
    tau_inv    =    inversegauss(ncol,Mu,Lambda1square) ;
    ...
    double lamshape1    =   ncol + a1_shape;
    double lamrate1     =   Rcpp::as<double>(Rcpp::wrap(.5*arma::accu(arma::pow(tau_inverse,-1)))) +b1_rate;


    lambda1_sq   =   R::rgamma(lamshape1,1/lamrate1);

    double lamshape2    =   ncol/2 + a2_shape ;
    double lamrate2     =   Rcpp::as<double>(Rcpp::wrap(.5*arma::accu(arma::pow(tau_inverse,-1)))) +b2_rate;


    lambda2   =   R::rgamma(lamshape2,1/lamrate2);


            Betahist.row(i)     = Beta1.t() ;
            Tausqhist.row(i)    = arma::pow(tau_inverse.t(),-1) ;
            sigma2hist[i]       = sigma2;
            Lambda1hist[i]      = sqrt(lambda1_sq) ;
...
return Rcpp::List::create(Rcpp::Named("Betahist")=Betahist,Rcpp::Named("Tausqhist")=Tausqhist,Rcpp::Named("sigma2hist")=sigma2hist,Rcpp::Named("lambda1hist")=Lambda1hist,Rcpp::Named("lambda2hist")=Lambda2hist);
