...
//[[Rcpp::depends(RcppArmadillo)]]
//[[Rcpp::export]]
Rcpp::List BEnfl(Rcpp::NumericMatrix X,Rcpp::NumericVector Y,double lambda1,double lambda2,double sigsc_hyper, double sigsh_hyper, int niter, int nburn,int nthin)
{
...
// 0. Variable Declaration
   //     RNGScope scope;
        ...
        double sigma2_scale;
        int N = niter/nthin;
        double lambda1_sq = lambda1*lambda1 ;
...
for (int i=0 ;i < niter ; i++)
    {
    dtau_inv   =    arma::diagmat(tau_inverse + lambda2*onecol) ;
    ...
    sigma2_shape    =(ncol + nrow -1)/2 + sigsc_hyper ;
    sigma2_scale    =Rcpp::as<double>(Rcpp::wrap(.5*(YY.t() -(XX*Beta1).t())*(YY -(XX*Beta1)) + .5*(Beta1.t()*(dtau_inv*Beta1)))) + sigsh_hyper;
